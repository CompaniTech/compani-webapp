<template>
  <ni-modal :model-value="modelValue" @hide="hide" @update:model-value="input">
    ...
    <div v-for="(line, index) in newPaymentPlan.priceLines" :key="index" class="price-line">
      ...
      <ni-button icon="close" :disable="newPaymentPlan.priceLines.length === 1" @click="removeLine(index)" />
    </div>
...
export default {
  ...
  props: {
    ...
    newPaymentPlan: { type: Object, default: () => ({ priceLines: [] }) },
  ...
  emits: ['hide', 'update:model-value', 'update:new-payment-plan', 'submit'],
  setup (props, { emit }) {
    const { newPaymentPlan, validations } = toRefs(props);
    ...
    const addLine = () => {
      emit('update:new-payment-plan', {
        ...newPaymentPlan.value,
        priceLines: [...newPaymentPlan.value.priceLines, { amount: null }],
      });
    };
    ...
    const removeLine = (index) => {
      emit('update:new-payment-plan', {
        ...newPaymentPlan.value,
        priceLines: newPaymentPlan.value.priceLines.filter((_, i) => i !== index),
      });
    };
    ...
    const updateAmount = (value, index) => {
      emit('update:new-payment-plan', {
        ...newPaymentPlan.value,
        priceLines: newPaymentPlan.value.priceLines.map((line, i) => (i === index ? { ...line, amount: value } : line)),
      });
    };
